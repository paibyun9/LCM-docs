...
function checkNonNegotiableDecision(user_message, lang = "ko") {
  ...
  const actOnBehalfPatterns = [
    ...
    // “대신 해지” 단독 요청 (신청/처리 같은 단어가 없어도 차단)
    /대신\s*(구독\s*)?해지.*(해줘|해주세요|해\s*줘|해라|해줄래|해\s*줄래)/i,
    /(구독\s*)?해지.*(내\s*대신|대신).*(해줘|해주세요|해\s*줘|해라|해줄래|해\s*줄래)/i,
    /you\s*(cancel|unsubscribe)\s*(it|this)?\s*for\s*me/i,
